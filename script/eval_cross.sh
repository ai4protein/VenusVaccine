...
# dataset=TumorBinary, BacteriaBinary, VirusBinary
dataset=VirusBinary
target_dataset=(BacteriaBinary TumorBinary)
pdb_type=ESMFold
...
for target_d in $target_dataset
do
    CUDA_VISIBLE_DEVICES=0 python eval.py \
        ...
        --return_attentions \
        --test_file dataset/$target_d/$pdb_type/test.json \
        --test_result_dir result/$plm_model/"$dataset"2"$target_d"/$seqs_type \
